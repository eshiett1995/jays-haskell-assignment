...
-- percfects interates through the 'int' param passed to it, starting from 1 to itself as seen '[1..digit]'
...
-- this iterates from 1 to a spefic number (n-1) and for each number in the iteration it performs the steps below,
-- 'n `mod` i' checks if the number is a divisor (mod is a haskell inbuilt function),
-- if it is a divsor, it would be added to the list using '1|'
-- 'sum []' sums all the divisors available for number 'n'
